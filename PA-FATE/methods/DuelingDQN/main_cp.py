...
def main():
    ...
    lb_metric = np.load(os.path.join(get_global_dict_value('dataset_conf')['dataset_path'], 'lb_metric.npy'), allow_pickle=True)[()]
    main_metric_type = get_global_dict_value('env_conf')['rwd']['metric']
    all_fea_lb_mean_auc = np.mean([lb_metric[lb]['all_fea'][main_metric_type] for lb in get_global_dict_value('dataset_conf')['tr_lb_list']])
    mainlog = MainLog()
    mainlog.record_conf()
    ...
    lb_mean_main_metric = 0
    max_iter_avg_lb_mean_main_metric = 0
    max_iter_avg_lb_mean_main_metric_iter = 0
    iter_avg_lb_mean_main_metric = 0

    tmp_valid_model = Network()
    for iter_id in range(get_global_dict_value('method_conf')['train_iter']):
            ...
            while True:
                ...
                if valid_global_ifdone == tr_lb_num:
                    ...
                    lb_mean_main_metric = np.mean([mainlog.lbs_info[tr_lb][main_metric_type + '_list'][
                                                   ...
                                                   for tr_lb in get_global_dict_value('dataset_conf')['tr_lb_list']])
                    lb_mean_main_metric_list.append(lb_mean_main_metric)
                    iter_avg_lb_mean_main_metric = np.mean(lb_mean_main_metric_list[-get_global_dict_value('method_conf')['iter_avg']:])
                    if iter_avg_lb_mean_main_metric > max_iter_avg_lb_mean_main_metric:
                        ...
                        max_iter_avg_lb_mean_main_metric_iter = iter_id - get_global_dict_value('method_conf')['cal_log_frequency']
        ...
        if get_global_dict_value('method_conf')['BIG']['use'] and iter_id % get_global_dict_value('method_conf')['BIG']['fq'] == 0 and iter_id > 0:
            p_si_array = 0
        ...
        if iter_id % get_global_dict_value('method_conf')['cal_log_frequency'] == 0 and iter_id > 0:
            ...
            max_lb_mean_main_metric = np.mean([mainlog.lbs_info[lb][main_metric_type + '_list'][
                                                   'max_' + main_metric_type] for lb in get_global_dict_value('dataset_conf')['tr_lb_list']])
            ...
            for lb in get_global_dict_value('dataset_conf')['tr_lb_list']:
                ...
                print(np.sum(cur_state), cur_state)
            print('pid', os.getpid(), 'seed', get_global_dict_value('method_conf')['seed'], 'cuda:' + str(get_global_dict_value('method_conf')['gpu_id']),
