...
class SubLog:
        ...
        if type == 'simulator':
            ...
            for metric_type in get_global_dict_value('dataset_conf')['metric_type_list']:
                ...
                if type2 == 'train':
                    ...
                    self.metric_list[metric_type + '_list'][metric_type] = np.zeros(
                        get_global_dict_value('method_conf')['further_train_iter'] + 1, dtype=np.float32)
                self.metric_list[metric_type + '_list']['max_' + metric_type + '_state'] = \
                    np.zeros(10, dtype=np.float32)
                self.metric_list[metric_type + '_list']['max_' + metric_type] = 0
        elif type == 'E_Tree':
            if type2 == 'train':
                self.E_Tree_lb = get_global_dict_value('dataset_conf')['tr_lb_list'][process_id]
            elif type2 == 'test':
                self.E_Tree_lb = get_global_dict_value('dataset_conf')['te_lb_list'][process_id]
            self.log_path = os.path.join(log_path, self.E_Tree_lb)
    ...
    def record_E_Tree_result(self, E_Tree_result):
        np.save(self.log_path + '/E_Tree_result.npy', E_Tree_result)

    def load_E_Tree_result(self, lb):
        try:
            tmp_dict = np.load(os.path.join(self.root_log_path, 'lb_E_Tree_result.npy'), allow_pickle=True)[()][lb]
        except:
            pass
        else:
            self.E_Tree_result = tmp_dict
